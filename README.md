...
| `/gallery`                       | 攝影牆。預設為可拖曳縮放的互動牆，可切換齊行清單（偏好存 localStorage）；SSR／無 JS／爬蟲取得簡化清單 | 視窗虛擬化 + 遠景 LOD；`<img srcSet>` 直連 R2 的 8 階 WebP，繞過 Vercel 圖片最佳化                                         |
| `/gallery/[slug]`                | 單張作品頁。拍攝時間、相機鏡頭、地點、HDR                                                             | ISR + `generateStaticParams`；LCP 圖以 `fetchPriority="high"` 載入，聚焦時載原檔                                           |
