...
import { getSemesters, type Semester } from '@/app/lib/supabase/schedule'
import {
  deleteFrom,
  getBusy,
  getPeople,
...
export default function TutoringPage() {
  ...
  // 課表工具換算來的，唯讀
  const [scheduleBusy, setScheduleBusy] = useState<BusySlot[]>([])
  // 證照輔導的級距看總時數，所以這一份不分月份
  const [licenseAll, setLicenseAll] = useState<Session[]>([])
  ...
  const refreshSessions = useCallback(
    async (target: string) => {
      const range = weeksOfMonth(target)
      const [list, license] = await Promise.all([
        getSessions(
          supabase,
          dateKey(range[0]),
          dateKey(addDays(range[range.length - 1], 6)),
        ),
  ...
  const refreshPeople = useCallback(
    async (semesterIds: string[]) => {
      // 每個學期的課表都讀進來，哪一天套哪一份由學期的起訖日期決定
      const [peopleList, busyList, scheduleLists] = await Promise.all([
        getPeople(supabase),
        getBusy(supabase),
      ...
      setBusy(busyList)
      setScheduleBusy(scheduleLists.flat())
    },
    [supabase],
  )
  ...
  useEffect(() => {
    getSemesters(supabase)
      .then(async (list) => {
        setSemesters(list)
        setSemester(list[0] ?? null)
        await refreshPeople(list.map((item) => item.id))
        setShare(await getShare(supabase))
      })
      .catch(() => toast.error(t('loadError')))
      .finally(() => setLoading(false))
  }, [supabase, refreshPeople])
  ...
  return (
    <div className={style.tutoring_page}>
      {!loading && <WeekPicker picker={picker} className={style.subhead} />}
