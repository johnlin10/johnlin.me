 ...
 */
function payloadOf(kind: QrKind, fields: Record<string, string>): string {
  return buildPayload(
    kind,
 ...
 * @returns 草稿
 */
function toDraft(code: QrCode): Draft {
  return {
    id: code.id,
    label: code.label,
    kind: code.kind,
...
export default function QrTool({ initial }: { initial: QrCode[] | null }) {
   ...
   */
  const figureOf = (code: Pick<Draft, 'kind' | 'fields'> & Style) => {
  ...
  /**
   * 畫不出來時提示原因：必填沒填完，還是資料塞不進一個 QR Code。
   * @param code 種類和欄位值
   */
  const complain = (code: Pick<Draft, 'kind' | 'fields'>) =>
    toast.error(payloadOf(code.kind, code.fields) ? t('form.tooLong') : t('form.empty'))

  const savePng = (code: Pick<Draft, 'label' | 'kind' | 'fields'> & Style) => {
    const figure = figureOf(code)
    if (!figure) return complain(code)
    // 整數倍放大，模組邊緣才不會糊掉
    const scale = Math.max(1, Math.round(PNG_SIZE / figure.side))
    const canvas = document.createElement('canvas')
    canvas.width = canvas.height = figure.side * scale
    const context = canvas.getContext('2d')
    if (!context) return complain(code)
  ...
  const saveSvg = (code: Pick<Draft, 'label' | 'kind' | 'fields'> & Style) => {
    const figure = figureOf(code)
    if (!figure) return complain(code)
  ...
  const actions: DataTableAction<QrCode>[] = [
    { label: t('downloadPng'), onClick: (code) => savePng(toDraft(code)) },
    { label: t('downloadSvg'), onClick: (code) => saveSvg(toDraft(code)) },
    { label: t('edit'), onClick: (code) => open(toDraft(code)) },
    ...
    <div className={style.page}>
      ...
      {codes.length === 0 ? (
        ...
        <DataTable
          ...
          actionsAs="menu"
          onRowClick={(code) => open(toDraft(code))}
        />
      )}
